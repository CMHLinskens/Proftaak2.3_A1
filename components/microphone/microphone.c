...
// #define MICROPHONETAG "Microphone"
...
//     esp_log_level_set(MICROPHONETAG, ESP_LOG_INFO);

//     audio_pipeline_handle_t pipeline;
//     audio_element_handle_t i2s_stream_reader, filter, raw_read;

//     ESP_LOGI(MICROPHONETAG, "[ 1 ] Start codec chip");
//     audio_board_handle_t board_handle = audio_board_init();
//     audio_hal_ctrl_codec(board_handle->audio_hal, AUDIO_HAL_CODEC_MODE_BOTH, AUDIO_HAL_CTRL_START);

//     ESP_LOGI(MICROPHONETAG, "[ 2 ] Create audio pipeline for recording");
//     audio_pipeline_cfg_t pipeline_cfg = DEFAULT_AUDIO_PIPELINE_CONFIG();
//     pipeline = audio_pipeline_init(&pipeline_cfg);
//     mem_assert(pipeline);

//     ESP_LOGI(MICROPHONETAG, "[2.1] Create i2s stream to read audio data from codec chip");
...
//     ESP_LOGI(MICROPHONETAG, "[2.2] Create filter to resample audio data");
...
//     ESP_LOGI(MICROPHONETAG, "[2.3] Create raw to receive data");
...
//     ESP_LOGI(MICROPHONETAG, "[ 3 ] Register all elements to audio pipeline");
...
//     ESP_LOGI(MICROPHONETAG, "[ 6 ] Start audio_pipeline");
//     audio_pipeline_run(pipeline);

// 	bool noError = true;
//     int16_t *raw_buff = (int16_t *)malloc(GOERTZEL_BUFFER_LENGTH * sizeof(short));
//     if (raw_buff == NULL) {
//         ESP_LOGE(MICROPHONETAG, "Memory allocation failed!");
//         noError = false;
//     }
...
//     ESP_LOGI(MICROPHONETAG, "[ 7 ] Destroy goertzel");
//     goertzel_free(configs);

//     ESP_LOGI(MICROPHONETAG, "[ 8 ] Stop audio_pipeline and release all resources");
